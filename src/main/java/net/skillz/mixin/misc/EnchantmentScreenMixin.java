    ...
    protected void drawBackgroundMixin(DrawContext context, float delta, int mouseX, int mouseY, CallbackInfo info) {
        /*if (LevelLists.enchantingTableList != null && !LevelLists.enchantingTableList.isEmpty()) {
            ...
            if (this.isPointWithinBounds(176, 0, 11, 13, (double) mouseX, (double) mouseY)) {
                ...
                String skill = Language.getInstance().get("spritetip.skillz.%s_skill".formatted(LevelLists.enchantingTableList.get(0)));
                list.add((Text.translatable("container.skillz.enchanting_tier", 1, skill, LevelLists.enchantingTableList.get(4))).formatted(Formatting.WHITE));
                list.add((Text.translatable("container.skillz.enchanting_tier", 2, skill, LevelLists.enchantingTableList.get(5))).formatted(Formatting.WHITE));
                list.add((Text.translatable("container.skillz.enchanting_tier", 3, skill, LevelLists.enchantingTableList.get(6))).formatted(Formatting.WHITE));
