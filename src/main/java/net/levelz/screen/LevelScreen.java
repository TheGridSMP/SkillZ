...
public class LevelScreen extends Screen implements Tab {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            context.drawTexture(LevelzMain.identifierOf("textures/gui/sprites/" + LevelManager.SKILLS.get(skillId).key() + ".png"), this.x + (i % 2 == 0 ? 11 : 99), this.y + 89 + i / 2 * 20, 0, 0, 16, 16, 16, 16);
            ...
            if (!this.attributes.isEmpty()) {
                if (this.showAttributes) {
                    ...
                    for (int i = this.attributeRow; i < this.attributeRow + maxAttributes; i++) {
                        String attributeKey = LevelzMain.getEntityAttributeIdAsString(this.attributes.get(i).getAttribute());
